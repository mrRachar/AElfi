    ...
    def parse_post(querystring, map_f=(lambda x: x)) -> odict:
        if os.environ.get('CONTENT_TYPE', '').startswith('multipart/form-data'):
            ...
            for arg in querystring.split(boundary)[1:-1]:
                ...
                if filename is None:
                    arg = nextline(arg)[:-1]
                    arg = arg.decode(config.charset) if arg.decode(config.charset)[-1] != '\r' else arg.decode(config.charset)[:-1]
                    name = name.decode(config.charset)[1:-1]
                    if name.endswith('[]'):
                        if name in post:
                            post[name].append(arg)
                        else:
                            post[name] = [arg]
                    else:
                        post[name] = arg
                    continue
                name, filename = name.decode(config.charset)[1:-1], filename.decode(config.charset)
                ...
                if name.endswith('[]'):
                    if name in post:
                        post[name].append(uploaded_file)
                    else:
                        post[name] = [uploaded_file]
                else:
                    post[name] = uploaded_file
            return post
        else:
